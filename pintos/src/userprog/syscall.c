...
/* In read system call, buffer should be valid.*/
void
...
/* In write system call, string should be in valid address. */
void
...
/* Map file in file descriptor at given address. */
int
mmap (int fd, void *addr)
{
  ...
  /* Check there is vm_entry in address. */
  ...
  /* Avoid file deletion, use reopen. */
  ...
  /* Assign mapping id and initialize mmap_file. */
  mmap->mapid = thread_current ()->next_pg++;
...
/* Unmap mmap_file by using mapping id. */
void
munmap (int mapping)
{
...
do_munmap (struct mmap_file *mmap_file)
{
  ...
  for (e = list_begin (vme_list); e != list_end (vme_list); e = list_next (e))
  {
    ...
    if (vme->is_loaded)
    {
      ...
      /* If page is dirty, record it in disk. */
...
/* After syscall_handler end, unpin vm_entry. */
void
unpin (void *addr)
{
...
/* Unpin function for string address. */
void
...
/* When read file, must pin file to avoid damage. */
void
