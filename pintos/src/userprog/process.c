...
start_process (void *file_name_)
{
  ...
  /* Initialize virtual memory hash table. */
...
process_exit (void)
{
  ...
  /* Close mmap_file and virtual memory hash table. */
  munmap (CLOSE_ALL);
  vm_destroy (&cur->vm);
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      if (vme == NULL)
        return false;
      vme->file = file_reopen(file);  /* File is deleted after load_segment, so use reopen. */
      vme->vaddr = upage;
      vme->offset = ofs;
...
handle_mm_fault (struct vm_entry *vme)
{
  ...
  /* Check this page complete loading. */
  vme->is_loaded = true;
  kpage->vme = vme;
  return true;
}

/* Expand stack for 8MB. */
bool
